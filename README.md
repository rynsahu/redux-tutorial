...
`javaScript const output = wrapInDiv(toLowerCase(trim(input)));`
...
`javaScript const transform = compose(wrapInDiv, toLowerCase, trim);`
...
`javaScript const transform = pipe(wrapInDiv, toLowerCase, trim);`
